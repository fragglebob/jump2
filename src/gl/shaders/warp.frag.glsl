...
void main() {
    ...
    // Input point
    vec3 X = vec3(vUv * u_size, mod(u_time, 576.0) * u_speed);

    // Evaluate noise
    vec4 noiseResult = bccNoiseDerivatives_XYBeforeZ(X);
    
    vec3 col = (noiseResult.xyz / 5.5);

    outColor = texture(u_texture, vUv + (col.xz * u_amount));
