...
import { GamepadManager } from "./gamepad/gamepad";
...
async function start() {
  ...
  const gamepad = setupGamepad();

  let glApp = new GLApp(mode, codeRoot, midiMix, gamepad);
  ...
  if (import.meta.hot) {
    import.meta.hot.accept("./gl/index.ts", (newGLModule) => {
      if (newGLModule) {
        glApp.stop();
        const newGLAppClass = newGLModule.GLApp as typeof GLApp;
        glApp = new newGLAppClass(mode, codeRoot, midiMix, gamepad);
        if (analyser) {
          glApp.setAudioAnalyser(analyser);
        }
        if (canvas && gl) {
